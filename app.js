...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const filterMonthInput = document.getElementById('filter-history-month');
    if (filterMonthInput) {
        filterMonthInput.addEventListener('change', renderHistory);
    }
    ...
    unsubscribe = db.collection(JOBS_COLLECTION)
        ...
        .onSnapshot((snapshot) => {
            jobs = snapshot.docs.map(doc => {
                const data = doc.data();
                if (data.empresa) {
                    const cleanEmpresa = data.empresa.trim().toUpperCase();
                    if (data.empresa !== cleanEmpresa) {
                        db.collection(JOBS_COLLECTION).doc(doc.id).update({ empresa: cleanEmpresa });
                        data.empresa = cleanEmpresa;
                    }
                }
                return {
                    id: doc.id,
...
function loadSettings(user) {
    db.collection('users').doc(user.uid).collection('settings').doc('pricing').get()
        .then((doc) => {
            if (doc.exists) {
                let rawConfig = doc.data();
                pricingConfig = {};
                let hasChanges = false;
                for (let key in rawConfig) {
                    let upperKey = key.toUpperCase();
                    if (key !== upperKey) hasChanges = true;
                    pricingConfig[upperKey] = rawConfig[key];
                }
                if (hasChanges) {
                    db.collection('users').doc(user.uid).collection('settings').doc('pricing').set(pricingConfig);
                }
...
window.promptAddCompany = function () {
    ...
    if (name && name.trim().length > 0) {
        addCompany(name.trim().toUpperCase());
    }
}
...
function renderHistory() {
    ...
    const uniqueEmpresas = [...new Set(finishedJobs.map(j => (j.empresa || '').trim().toUpperCase()).filter(Boolean))].sort();
    ...
    finishedJobs = finishedJobs.filter(job => {
        ...
        if (filterMonth && job.data_inicio) {
            const [y, m] = filterMonth.split('-');
            const jobDate = job.data_inicio.split('-');
            if (jobDate[0] !== y || jobDate[1] !== m) return false;
        }
    ...
    window.currentFilteredJobs = finishedJobs;
            ...
            card.innerHTML = `
                <div class="job-header" style="display:flex; align-items:center; gap:10px;">
                    ...
                    <div style="flex:1; display:flex; justify-content:space-between; align-items:center;">
                        <span class="job-title" style="color:var(--primary); font-size:1.25rem;">${job.empresa || ''}</span>
...
window.getJobsToExport = function() {
    const visibleJobs = window.currentFilteredJobs || [];
    if (selectedJobs.size > 0) {
        return visibleJobs.filter(j => selectedJobs.has(j.id));
    }
    return visibleJobs;
};

window.generateReport = function () {
    const reportJobs = window.getJobsToExport();
    
    if (reportJobs.length === 0) {
        alert("Nenhum serviço sendo exibido. Filtre algo antes de exportar.");
        return;
    }
    ...
    reportJobs.forEach(job => {
        ...
        const dateParts = job.data_inicio ? job.data_inicio.split('-') : ['','',''];
        const dateBr = dateParts.length === 3 ? `${dateParts[2]}/${dateParts[1]}` : '';
...
window.exportToExcel = function() {
    ...
    if (reportJobs.length === 0) {
        alert("Nenhum serviço sendo exibido na tela.");
        return;
    }
    ...
    link.setAttribute("download", `Relatorio_Exportado.csv`);
...
window.selectAllVisible = function() {
    ...
    // Toggle: if all are selected, deselect all. Else, select all.
    const allSelected = visibleJobs.every(j => selectedJobs.has(j.id));
    
    if (allSelected) {
        visibleJobs.forEach(j => selectedJobs.delete(j.id));
    } else {
        visibleJobs.forEach(j => selectedJobs.add(j.id));
    }
    renderHistory();
};
