...
const TIPOS = ["ROUBO/FURTO", "VERIFICAÇÃO", "ALARME", "ANTENA", "APOIO"];
...
function setupForm() {
    entryForm.addEventListener('submit', async (e) => {
        ...
        try {
            const newJob = {
                uid: currentUser.uid, // OWNER ID
                email: currentUser.email, // Backup ref
            ...
            // Compat Syntax: db.collection().add()
            await db.collection(JOBS_COLLECTION).add(newJob);
        ...
        card.innerHTML = `
            ...
            <div class="job-details">
                ...
                <div style="margin-top:16px;">
                    <label style="font-size:0.8rem; font-weight:600; color:var(--text-muted); text-transform:uppercase;">KM Total na Chegada</label>
                    <input type="number" id="km-${job.id}" placeholder="KM do Odômetro" style="margin-top:4px;" value="${job.km_rodado || ''}">
                </div>
            </div>
            <div style="display:flex; gap:12px; margin-top:16px;">
                <button class="btn-primary" style="margin-top:0; flex:1; background:var(--success);" onclick="window.finishJob('${job.id}')">
                    FINALIZAR
                </button>
...
window.finishJob = async function (id) {
    ...
    try {
        ...
        let kmVal = 0;
        if (kmInput) kmVal = parseFloat(kmInput.value) || 0;
