...
const DEFAULT_PRICING = {
    "OnSystem": { "ROUBO/FURTO": 250, "default": 200 },
    "RVS": { "ROUBO/FURTO": 200, "VERIFICAÇÃO": 100, "default": 150 },
    "C&C": { "default": 200 },
    "Sancor": { "default": 250 },
    "Carsystem": { "default": 200 }
};
...
window.saveSettings = function () {
    ...
    const companies = ["OnSystem", "RVS", "C&C", "Sancor", "Carsystem"]; // Or fetch from DOM if dynamic
    const newConfig = {};

    companies.forEach(company => {
        newConfig[company] = {};
        TIPOS.forEach(tipo => {
            const val = parseFloat(document.getElementById(`price-${company}-${tipo}`).value);
            if (!isNaN(val)) {
                newConfig[company][tipo] = val;
            }
        });
        // Default fallback per company
        const defVal = parseFloat(document.getElementById(`price-${company}-default`).value);
        if (!isNaN(defVal)) newConfig[company]['default'] = defVal;
    });
...
function renderSettings() {
    ...
    container.innerHTML = '';
    const companies = ["OnSystem", "RVS", "C&C", "Sancor", "Carsystem"];

    companies.forEach(company => {
        const wrapper = document.createElement('div');
        wrapper.className = 'job-card'; // Reuse card style
        wrapper.style.marginBottom = '12px';

        let html = `<h4 style="border-bottom:1px solid var(--border); padding-bottom:8px; margin-bottom:12px;">${company}</h4>`;
        ...
        // Per Type inputs (Accordion style or list)
...
Passando para lembrar do pagamento referente ao serviço da placa *${job.placa}* (${job.modelo}), realizado em ${formatDate(job.data_inicio)}.
