...
function renderHistory() {
    ...
    if (searchPlaca) {
        finishedJobs = finishedJobs.filter(j => (j.placa || '').toUpperCase().includes(searchPlaca));
    }

    if (finishedJobs.length === 0) {
        historyListEl.innerHTML = `<div style="text-align:center; padding:40px; color:var(--text-muted);"><p>Nenhum histórico disponível no momento.</p><p style="font-size:0.8rem; margin-top:10px;">(Diagnóstico: Buscando ${jobs.length} registros totais armazenados para este usuário.)</p></div>`;
        return;
    }
...
window.generateOccurrenceReport = function (id) {
    ...
    const msg = `🧾 *RELATÓRIO DE OCORRÊNCIA*
